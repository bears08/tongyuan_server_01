$(function() {
    ...
    $('#changeStatusBtn').click(function() {
        var rows = $('#gridResult').datagrid('getChecked');
        if (rows.length == 0) {
            $.messager.alert('警告', '请选择至少一条记录信息');
            return;
        }
...
function showGrid() {
    ...
    $('#gridResult').datagrid({
        ...
        url : $("#rootPath").val()+"/finance/exmanager/orderStatus/do_search?tradeType=ransom",
        method:'post',
        pagination:true,
        rownumbers:false,
...
function changeOrderStatus(){
    ...
    $.ajax({
        url: $('#rootPath').val() + '/finance/exmanager/orderStatus/changeStatus?tradeType=ransom',
        dataType: 'JSON',
        data: {
            orderIds: orderIds,
        ...
        success: function(data) {
            ...
            $('#gridResult').datagrid('clearChecked');
