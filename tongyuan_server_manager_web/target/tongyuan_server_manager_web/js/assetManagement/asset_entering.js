	...
	$('#gridResult').datagrid({
		...
		columns : [ [ // 定义每一列的字段.
		{
            ...
            formatter: function(value,row,index){
                var status;
                var periodUnit = row.periodUnit;
                if(periodUnit==null || periodUnit=='')return value;
			...
			formatter: function(value,row,index){
				...
				if(row.status != 2 && row.status != 1 && row.status != 3){
					...
					htmlStr+="<a href='javascript:void(0);' style='font-size: 11px;color: blue;' onclick='delAssetInfo(\""+row.id+"\")'>删除</a>";
				}
				return htmlStr;
			}
		}] ],
...
function doEnteringExcelTemplate(){
	...
	if(suffix == "xls" || suffix == "xlsx"){
		$('#uploadFileForm').form('submit', {
			url:$("#rootPath").val()+"/finance/assetManagement/entering/addBatch?suffix="+suffix,
		    success: function(data){
