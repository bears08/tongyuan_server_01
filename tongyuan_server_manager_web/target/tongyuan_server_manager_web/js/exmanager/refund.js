        ...
        columns : [[ // 定义每一列的字段.
                {
                    ...
                    formatter : function(value, row, index) {
                        var status = "";
                        var value = row.payStatus;
                        if (value == "0" || value == "3") {//必须是支付成功或者退款失败,并且核保失败的才显示退款按钮
                            
                            if(row.IsSuccess == false || row.IsSuccess == null){
                                status = "<button onclick=\"refund(\""+row.orderId+"\")\">退款</button>";
                            }else{
